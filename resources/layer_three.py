...
def layer_three(layer_two_file):
    ...
    for i in range(len(df_layer_two_file["df_title_url"])):
        ...
        # Obtain link
        paper_title_links = driver.find_elements_by_css_selector(".rendering_contributiontojournal_publicationaccessrenderer > ul.dois > li > div > a")
        paperlink = [el.get_attribute("href") for el in paper_title_links]
        print("Pasting paper url")
        data['df_paper_url'] = paperlink

        # Obtain Abstract
        paper_abstract = driver.find_elements_by_css_selector(".rendering_abstractportal.rendering_contributiontojournal_abstractportal > div")
        paper_abstracts = [el.text for el in paper_abstract]
        print("Pasting paper abstract")
        data['df_paper_abstract'] = paper_abstracts

        # Obtain authors
        paper_authors = driver.find_elements_by_css_selector(".rendering_contributiontojournal_associatespersonsclassifiedportal > p > a:nth-child(1) > span")
        ...
        paper_publication_dates = driver.find_elements_by_css_selector(".rendering_contributiontojournal_detailsportal > div > table > tbody > tr.status > td > span.date")
